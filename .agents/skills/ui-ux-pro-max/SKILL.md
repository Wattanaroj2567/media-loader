...
> **Note:** On Windows, use `python` instead of `python` to run scripts (e.g., `python scripts/search.py` instead of `python scripts/search.py`).
...
```bash
python .agents/skills/ui-ux-pro-max/scripts/search.py "list performance navigation" --stack react-native
```
