    ...
    <main className="relative min-h-dvh overflow-hidden bg-bg-base px-4 py-5 text-foreground sm:px-6 lg:px-10">
      ...
      <div className="relative mx-auto grid min-h-[calc(100dvh-2.5rem)] max-w-7xl items-center gap-12 lg:grid-cols-[1.02fr_0.98fr] lg:gap-16">
        <section className="max-w-3xl">
          ...
          <p className="mt-5 max-w-2xl text-base leading-8 text-text-muted animate-fade-in-up [animation-delay:400ms]">
            {t("landing.subtitle")}
          </p>

          {/* 5. CTA container (delay 500ms) */}
          <div className="mt-8 flex flex-col gap-3 sm:flex-row animate-fade-in-up [animation-delay:500ms]">
            <Button
              type="button"
              size="lg"
              onClick={handleLogin}
          ...
          <div className="mt-4 flex flex-wrap items-center gap-x-2 gap-y-1 text-xs text-text-dim animate-fade-in-up [animation-delay:600ms]">
          ...
          <div className="mt-1.5 flex flex-wrap items-center gap-x-2 gap-y-1 text-xs text-text-dim animate-fade-in-up [animation-delay:650ms]">
          ...
          <div className="mt-8 grid gap-3 sm:grid-cols-3">
            {assurances.map((item, index) => (
              ...
              >
                ...
                <p className="text-sm font-medium leading-normal text-text-muted sm:mt-3 sm:leading-relaxed">{item}</p>
              </div>
            ))}
          </div>
        </section>
          ...
          <div className="rounded-[1.5rem] border border-border bg-bg-base/35 p-4 sm:p-6">
            ...
            <div className="mt-8 grid gap-3">
              {steps.map((step, index) => (
                <div
                  key={step}
                  className="flex gap-3 sm:gap-4 rounded-2xl border border-border bg-bg-surface/50 p-3 sm:p-4"
                >
                  <span className="grid size-8 shrink-0 place-items-center rounded-xl border border-primary/20 bg-primary/10 font-mono text-xs text-primary">
